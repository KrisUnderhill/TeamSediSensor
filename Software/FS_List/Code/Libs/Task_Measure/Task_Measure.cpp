...
void TaskMeasure::init(){
    ...
    timerAlarmWrite(timer, 5000000, true); /* Timer fires every 15,000,000 us (15s), repeat true */
...
void TaskMeasure::run(){
    if (xSemaphoreTake(timerSemaphore, 0) == pdTRUE){
        ...
        taskB.setBuffer((uint8_t*)timeCStr, strlen(timeCStr)-1); /* -2 : I don't want the \r\n char or the \0 end */
